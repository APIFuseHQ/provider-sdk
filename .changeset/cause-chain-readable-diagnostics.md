...
Each cause frame now retains its existing class, optional code, original message length, and fingerprint while also carrying a redacted `message`. The shared diagnostic sanitizer removes personal and credential material, preserves correlation identifiers and useful URL provenance, encodes control characters, and visibly truncates hostile upstream text to the existing 300-character diagnostic bound. The public response remains unchanged.
