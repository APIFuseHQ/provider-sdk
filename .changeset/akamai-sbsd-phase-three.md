...
engine-owned, AEAD-encrypted egress lease that rebinds the exact selected
...
an unreadable body fails with a typed error. This is a major API change because
the new method is required on the public `StealthSession` interface and existing
structural implementations must add it.
