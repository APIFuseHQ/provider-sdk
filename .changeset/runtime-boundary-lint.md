...
- `node-runtime-module-import` — value imports of `fs`, `fs/promises`, `net`, `tls`, `dgram`, `http`, `https`, `http2`, `child_process` (with or without `node:`) through `import`/`export … from`, `import =`, `require()`, or dynamic `import()`, plus `Bun.spawn`, `Bun.spawnSync`, ``Bun.$` ` ``, `Bun.file`, `Bun.write`. Type-only imports are ignored — clause-level `import type` and a named import whose every binding is inline `type` (`import { type ChildProcess } from "node:child_process"`), both erased before the module loads; `node:path`, `node:crypto`, `node:url`, `node:buffer` are not in the set.
- `direct-fetch-call` — a call to the global `fetch` (bare identifier or `globalThis`/`window`/`self`/`global` member). `ctx.stealth.fetch()` is a member call and never matches; a bare `fetch` call whose enclosing scope declares a `fetch` binding (parameter, variable, import, function, catch clause) is calling that binding, not the global, and is left alone — the check is lexical, so a wrapper taking `fetch` as a parameter does not hide a global `fetch()` elsewhere in the same file.
