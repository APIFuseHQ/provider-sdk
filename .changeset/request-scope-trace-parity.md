...
Streaming request roots now remain active through terminal success, failure, or cancellation before emitting one outcome log, with client cancellation classified as a failed `client_cancelled` request. Provider logs keep the served `providerId` authoritative and include `requestedProviderId` only for mismatched auth routing requests.
