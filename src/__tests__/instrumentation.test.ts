...
describe("createTraceContext", () => {
	...
	it("exports only spans that were not included in an earlier OTLP batch", async () => {
		...
		expect(exportedTraceIds[1]).toBe(exportedTraceIds[0]);
	});

	it("retries a failed OTLP batch once and identifies the affected request", async () => {
		const originalFetch = globalThis.fetch;
		...
		globalThis.fetch = Object.assign(
			async (_input: string | URL | Request, init?: RequestInit) => {
				callCount += 1;
				attempts.push(
					JSON.parse(String(init?.body)).resourceSpans[0].scopeSpans[0].spans.map(
						(span: { name: string }) => span.name,
					),
				);
				if (callCount === 1) throw new Error("collector unavailable");
				return new Response(null, { status: 200 });
			},
			{ preconnect: originalFetch.preconnect },
		);
		...
		try {
			...
			await trace.span("handler", async () => undefined);
		...
		expect(warn.mock.calls[0]?.[0]).toContain("request_id=retry-request");
		expect(warn.mock.calls[0]?.[0]).toContain("batch=");
	});
});
