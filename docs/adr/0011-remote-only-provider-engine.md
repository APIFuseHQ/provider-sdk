...
| D2 | **A provider process keeps only: the declaration, operation handler logic, ambient `trace`, and handle field logic (`kind.field()`, `pick`); handle storage is engine-owned `state`.** | This is the "contract and mapping only" provider ADR-0010 described; the measurement shows nothing else belongs there. |
...
7. **Wording amended by ADR-0012 (2026-09-08).** Inline-mode `choice` no longer exists; D2 now reads "handle field logic", and the server-mode choice token master secret listed in D3 is gone because bound handles are isolated by connection scope, not by a derived hash. See `docs/adr/0012-llm-friendly-handles.md`.
