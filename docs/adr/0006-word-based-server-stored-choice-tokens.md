...
- Date: 2026-08-10; amended 2026-08-19, 2026-08-20 and 2026-09-10 (legacy sunset; bound-verified stale disclosure; operation rate-limit declaration deleted)
...
2. Providers should declare `annotations.rateLimit` on operations that parse choices. The platform already supports this: for example, daangn `search-regions` declares `calls: 10` per minute, throttling per-tenant guessing to approximately 0.17 requests per second.
   ...
   **Amended 2026-09-10 (the operation-level declaration was deleted; the throttle moved to the gateway and to `ctx.request.tenantId`).** This requirement can no longer be met by declaration. `annotations.rateLimit` was declarable when this ADR was accepted, but nothing enforced it — ADR-0009's measured consumer audit scores it `registry 0 / gateway 0 real matches` ("dead — every gateway match is unrelated") and ADR-0012 records that 14 of the 20 token-parsing repositories carried no declaration while the field still existed — and **ADR-0009 D8 deleted the field**, so the daangn example above no longer exists and no provider can restate it (`rateLimit` survives only in the legacy codemod list, `src/cli/migrate-operation-declaration.ts:37`). Two defenses replace it. (a) The gateway rate limits every request per minute against the caller's policy class (`PolicyClass.RateLimitPerMinute`), keyed **per subject** — organization id, tenant id, or service-account id — so a guesser is bounded to its own budget and cannot spend another principal's. (b) A provider that needs a per-principal bound on an unbound namespace (for example a cap on outstanding public search cursors per tenant) keys it on `ctx.request.tenantId`, the gateway-asserted principal scope on the operation envelope (organization id for customer subjects, service-account id for platform service accounts; `undefined` on direct calls; issue #164). That cap is a soft cap built on `ctx.state.namespace(name, { scope: "provider", ... }).compareAndSet(...)`: `compareAndSet` is a member of `ProviderStateNamespace`, and `increment` is not usable as a quota primitive because it is get-then-set on the Redis backend and unsupported by the in-memory one. Bounding a shared `maxEntries` namespace per tenant is otherwise impossible, because every connectionless request shares one `scope:connection:missing` state scope. The §2 numbers are not recalculated: they assume an attacker sustaining 50 requests per second, and any policy-class budget at or below that leaves the analysis conservative.
3. Unbound handles minimize TTL. When an existing contract requires a long-lived unbound handle, it uses five-word `high` strength and an operation rate limit rather than relying on consumption.
   ...
   **Amended 2026-09-10.** "an operation rate limit" now means the gateway policy-class budget and, where a provider needs one, the per-principal cap of requirement 2; the operation-level declaration this sentence referred to was deleted by ADR-0009 D8.
