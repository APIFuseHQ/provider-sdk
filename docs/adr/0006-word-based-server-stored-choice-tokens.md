...
2. Online guessing is throttled by the gateway, not by the provider: every tenant request is rate limited per minute by its policy class (gateway `PolicyClass.RateLimitPerMinute`), so a guesser is bounded to that class's budget. There is no `annotations.rateLimit` operation declaration and no provider declares one; an earlier revision of this ADR claimed otherwise. Providers that need a per-principal bound on an unbound namespace (for example a cap on outstanding public search cursors per tenant) key it on `ctx.request.tenantId` — the gateway-asserted principal scope on the operation envelope (organization id for customers, service-account id for platform service accounts; `undefined` on direct calls). The cap is a soft cap built on `ProviderRuntimeState.compareAndSet` in a provider-scoped namespace: `increment` is not atomic on Redis and unsupported by the in-memory backend. Bounding a shared `maxEntries` namespace per tenant is otherwise impossible, because every connectionless request shares one `scope:connection:missing` state scope.
