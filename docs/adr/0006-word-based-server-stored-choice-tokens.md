...
The legacy parser cannot be removed on a calendar date chosen independently of issued TTLs. The former 2026-09-15 removal date is withdrawn. Rollout records the last time any deployed SDK can mint a legacy managed token, `T_last_legacy_mint`, and the parser removal milestone MUST be later than `T_last_legacy_mint + TTL_max`. The live-provider inventory found a maximum managed-token TTL of 180 days (NOL booking tokens), so the compatibility window is at least 180 days after the last legacy mint unless those tokens are explicitly invalidated or rehydrated. A token is accepted according to its issuer TTL measured from issuance; this horizon describes only when parser code may be removed and is not a promise that every legacy token remains valid until a named date.
