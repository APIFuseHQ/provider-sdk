...
const NEGATIVE_CONTROLS = [
	...
	{
		filename: "negative-control-resolver-telemetry-description.ts",
		expectedCode: "TS2353",
		description: "resolver gateway telemetry rejects vendor error descriptions",
		source: [
			'import type { ResolverTelemetryHeaderPayload } from "@apifuse/provider-sdk";',
			"",
			"export const mustNotCompile: ResolverTelemetryHeaderPayload = {",
			"\tattempts: 0,",
			"\tfailovers: 0,",
			"\tvendorChain: [],",
			"\tpollCount: 0,",
			'\tvendorErrorDescription: "free text must stay log-only",',
			"};",
			"",
		].join("\n"),
	},
	{
		filename: "negative-control-resolver-telemetry-phase.ts",
		expectedCode: "TS2322",
		description: "resolver gateway telemetry rejects phases outside its closed vocabulary",
		source: [
			'import { closedEnum, type ResolverTelemetryHeaderPayload } from "@apifuse/provider-sdk";',
			"",
			"export const mustNotCompile: ResolverTelemetryHeaderPayload = {",
			"\tattempts: 1,",
			"\tfailovers: 0,",
			...
			"\tpollCount: 0,",
			'\tattemptSamples: [{ v: closedEnum("hypersolutions"), p: closedEnum("solve_exchange"), o: closedEnum("error"), ms: 1 }],',
			"};",
			"",
		].join("\n"),
	},
	{
		filename: "positive-control-resolver-telemetry-contributor.ts",
		expectedCode: "",
		description: "resolver telemetry contributor satisfies the gateway-ingestible contract",
		source: [
			'import { closedEnum, ResolverTelemetryCollector, type GatewayIngestible, type ResolverTelemetryHeaderPayload, type ResolverTelemetryLogPayload, type ResolverTelemetryPhase, type TelemetryContributor } from "@apifuse/provider-sdk";',
			"",
			"export const resolver: TelemetryContributor<ResolverTelemetryLogPayload, ResolverTelemetryHeaderPayload> = new ResolverTelemetryCollector();",
			'const hyperPhases: ResolverTelemetryPhase[] = ["measure_ip", "fetch_script", "generate_payload", "post_payload"];',
			'export const hyperPhaseHeader: ResolverTelemetryHeaderPayload = { attempts: 1, failovers: 0, vendorChain: [closedEnum("hypersolutions")], pollCount: 0, attemptSamples: hyperPhases.map((phase) => ({ v: closedEnum("hypersolutions"), p: closedEnum(phase), o: closedEnum("ok"), ms: 1 })) };',
			"declare const header: ResolverTelemetryHeaderPayload;",
			"export const ingestible: GatewayIngestible<ResolverTelemetryHeaderPayload> = header;",
			"",
		].join("\n"),
	},
	{
	...
	},
	{
		...
		source: [
			'import type { HttpTelemetryHeaderPayload, GatewayIngestible } from "@apifuse/provider-sdk";',
			"type UnsafeHttpHeader = HttpTelemetryHeaderPayload & { message: string };",
			"declare const header: HttpTelemetryHeaderPayload;",
			'export const mustNotCompile: GatewayIngestible<UnsafeHttpHeader> = { ...header, message: "upstream free text" };',
			"",
		].join("\n"),
	},
	{
		filename: "positive-control-http-telemetry-contributor.ts",
		expectedCode: "",
		description: "HTTP contributor satisfies the gateway and tenant projection contracts",
		source: [
			'import { HttpTelemetryCollector, type HttpTelemetryHeaderPayload, type HttpTelemetryLogPayload, type HttpTelemetryRetryPayload, type GatewayIngestible, type TenantNeutral, type TelemetryContributor } from "@apifuse/provider-sdk";',
			'import { HttpTelemetryCollector as ServerHttpCollector } from "@apifuse/provider-sdk/server";',
			"export const http: TelemetryContributor<HttpTelemetryLogPayload, HttpTelemetryHeaderPayload> = new HttpTelemetryCollector();",
			"declare const header: HttpTelemetryHeaderPayload;",
			"export const ingestible: GatewayIngestible<HttpTelemetryHeaderPayload> = header;",
			"export const retry: TenantNeutral<HttpTelemetryRetryPayload> | undefined = new ServerHttpCollector().toTenantRetryPayload();",
			"",
		].join("\n"),
	},
	{
